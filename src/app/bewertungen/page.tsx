...
const BewertungPage = () => {
  ...
  useEffect(() => {
    // Nachricht anzeigen, dass die Daten analysiert sind und Fragen gestellt werden können
    setMessages([{ role: 'bot', content: 'Die Bewertungsdaten wurden analysiert. Du kannst nun Fragen dazu stellen.' }]);
  }, []);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result = await axios.post('/api/bewertung', {
        query,
        conversationHistory: messages.filter(msg => msg.role === 'user').map(msg => ({ question: msg.content, answer: '' })),
      });
      const botMessage = { role: 'bot', content: result.data.response };
      setMessages((prevMessages) => [...prevMessages, botMessage]);
    } catch (error) {
      console.error('Fehler beim Abrufen der Bewertung:', error);
      setMessages((prevMessages) => [...prevMessages, { role: 'bot', content: 'Es gab ein Problem bei der Analyse der Bewertungen.' }]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex items-center justify-center h-screen w-screen">
      <div className="bg-white p-6 rounded-lg border border-[#e5e7eb] w-[440px] h-[634px] flex flex-col" style={{ boxShadow: "0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05)" }}>
        ...
        <div className="flex flex-col space-y-1.5 pb-6">
          <h2 className="font-semibold text-lg tracking-tight">Bewertungs Bot</h2>
          ...
          {messages.map((msg, index) => (
            <div key={index} className={`flex gap-3 my-4 text-gray-600 text-sm ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <p className="leading-relaxed">
                <span className="block font-bold text-gray-700">{msg.role === 'user' ? 'Ich' : 'BewertungsBot'} </span>
                {msg.content}
              </p>
            </div>
          ))}
        ...
        <div className="flex items-center pt-4">
          <form className="flex items-center justify-center w-full space-x-2" onSubmit={(e) => { e.preventDefault(); handleAnalyze(); }}>
            <input
              ...
              placeholder="Stelle deine Bewertungsfrage..."
              value={query}
